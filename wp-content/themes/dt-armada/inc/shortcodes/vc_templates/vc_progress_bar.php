...
if ( 'custom' === $bgcolor && '' !== $custombgcolor ) {
	...
	$custombgcolor = '';
	$customtxtcolor = '';
	$el_class .= ' vc_progress-bar-color-' . esc_attr( $bgcolor );
}
...
// label above
$css_class .= ( 'top' === $caption_pos ? ' vc-label-above' : '' );
...
foreach ( $graph_lines_data as $line ) {
	...
	$label = '<small class="vc_label"' . $line['txtcolor'] . '>'. $line['label'] . $unit .'</small>';

	$output .= 'top' === $caption_pos ? $label : '';

	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ? ' vc_progress-bar-color-' . $line['color'] : '' )
		. ( ( 'accent-bg' === $bgcolor && ( ! isset( $line['color'] ) || 'custom' !== $line['color'] ) ) ? ' accent-bg' : '' )
		. '">';

	$output .= 'on' === $caption_pos ? $label : '';
