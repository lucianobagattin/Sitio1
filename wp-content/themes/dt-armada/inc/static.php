...
if ( ! function_exists( 'presscore_enqueue_scripts' ) ) :
	...
	function presscore_enqueue_scripts() {
		...
		// in header
		presscore_enqueue_theme_script( 'dt-above-the-fold', 'js/above-the-fold', array( 'jquery' ), $theme_version, false );
		...
		$dt_local['ajaxurl'] = isset( $dt_local['ajaxurl'] ) ? esc_url( $dt_local['ajaxurl'] ) : admin_url( 'admin-ajax.php' );

		// add some additional data
		wp_localize_script( 'dt-above-the-fold', 'dtLocal', $dt_local );
...
if ( ! function_exists( 'presscore_body_class' ) ) :
	...
	function presscore_body_class( $classes ) {
		...
		switch ( $config->get( 'buttons.style' ) ) {
			...
			case 'flat':
			default: $classes[] = 'btn-flat'; break;
		}
