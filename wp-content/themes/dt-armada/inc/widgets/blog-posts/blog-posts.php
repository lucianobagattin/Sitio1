...
class Presscore_Inc_Widgets_BlogPosts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $terms ) {
			...
			$list_args = array(
				'show_images' => (boolean) $instance['thumbnails'],
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'images_dimensions' ); ?>"><?php _ex('Featured images proportions:', 'widget', LANGUAGE_ZONE); ?>
			...
			<input type="text" size="3" name="<?php echo $this->get_field_name( 'images_dimensions' ); ?>[h]" value="<?php echo esc_attr( $instance['images_dimensions']['h'] ); ?>" /><span><?php _ex( 'in px', 'widget', LANGUAGE_ZONE ); ?></span>
		</p>
