...
if ( ! function_exists( 'presscore_add_presets' ) ) :
	...
	function presscore_add_presets( $presets = array() ) {
		...
		foreach( $presets_names as $preset_name ) {
			...
			foreach( array( 'b', 'c', 'r', 's' ) as $header_type ) {
...
if ( ! function_exists( 'presscore_flush_rewrite_rules' ) ) :
...
if ( ! function_exists( 'presscore_dt_paginator_args_filter' ) ) :
	...
	function presscore_dt_paginator_args_filter( $args ) {
		...
		$args['wrap'] = '<div class="%CLASS%" role="navigation"><div class="page-links">%LIST%</div>%PREV%%NEXT%</div>';
		$args['pages_wrap'] = '';
		...
		$args['act_class'] = 'act';
		$args['next_text'] = _x( 'next', 'paginator', LANGUAGE_ZONE );
		$args['prev_text'] = _x( 'prev', 'paginator', LANGUAGE_ZONE );
		$args['no_next'] = '<a href="#" class="nav-next disabled">' . $args['next_text'] . '</a>';
		$args['no_prev'] = '<a href="#" class="nav-prev disabled">' . $args['prev_text'] . '</a>';
