...
class Presscore_PhotoScroller extends Presscore_Slider {
	...
	protected function get_slide( &$slide ) {
		...
		$slide_image .= $this->get_slide_video_link( $slide );
	...
	protected function get_slide_buttons( &$slide ) {
		...
		if ( $show_slide_link ) {
			$slide_buttons .= '<a href="' . esc_url( $slide->link ) . '" class="dt-btn-m btn-project-link" target="_blank"></a>';
		}

		if ( ! empty( $slide->share_icons ) ) {
			$slide_buttons .= $slide->share_icons;
		}

		if ( $slide_buttons ) {
			$slide_buttons = '<div class="album-content-btn">' . $slide_buttons . '</div>';
		}
	...
	protected function get_slide_share_buttons() {
		...
		$place = 'photo';
		$html = '';
		$share_buttons = presscore_get_share_buttons_list( $place );
		if ( ! empty( $share_buttons ) ) {

			$share_button_title = of_get_option( "social_buttons-{$place}-button_title", '' );

			$html .= '<div class="album-share-overlay">';
				$html .= '<a  class="share-button entry-share dt-btn-m" href="#">' . esc_html( $share_button_title ) . '</a>';
