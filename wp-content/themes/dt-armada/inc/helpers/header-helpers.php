...
if ( ! function_exists( 'presscore_get_header_class' ) ) :
	...
	function presscore_get_header_class( $class = '' ) {
		...
		if ( $config->get( 'header.background.fixed' ) ) {
			$classes[] = 'bg-fixed';
		}

		if ( of_get_option( 'submenu-parent_clickable', true ) ) {
...
if ( ! function_exists( 'presscore_get_header_elements_list' ) ) :
	...
	function presscore_get_header_elements_list( $field_name ) {
		...
		switch ( $header_layout ) {
			case 'side' :
				$fields = of_get_option('header-side_layout_elements', array());
				break;
			case 'center' :
				$fields = of_get_option('header-center_layout_elements', array());
				break;
			case 'classic' :
				$fields = of_get_option('header-classic_layout_elements', array());
				break;
			default : // left
				$fields = of_get_option('header-left_layout_elements', array());
		}
