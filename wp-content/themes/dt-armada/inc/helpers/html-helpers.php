...
if ( ! function_exists( 'presscore_main_container_classes' ) ) :
	...
	function presscore_main_container_classes( $custom_class = array() ) {
		...
		if ( ! of_get_option( 'sidebar-divider-vertical', true ) ) {
	...
	function presscore_post_details_link( $post_id = null, $class = array('details', 'more-link'), $link_text = null ) {
		...
		if ( $url ) {
			$output = sprintf(
				...
				is_string( $link_text ) ? $link_text : __( 'Read details', LANGUAGE_ZONE )
			);
		}
...
if ( ! function_exists( 'presscore_display_share_buttons' ) ) :
	...
	function presscore_display_share_buttons( $place = '', $options = array() ) {
		...
		$button_title = esc_html( $options['title'] );
		$link_class = '';
		if ( ! $button_title ) {
			$button_title = __( 'Share this', LANGUAGE_ZONE );
			$link_class = ' no-text';
		}

		$html = '';

		$html .= '<a  class="share-button entry-share dt-btn-m' . $link_class . '" href="#">' . $button_title . '</a>';
		...
		<div class="' . esc_attr(implode(' ', $class)) . '">
			<div class="wf-table">
				<div class="wf-td">
					<h3>' . $button_title . '</h3>
					<h2 class="h1-size">' . $options['page_title'] . '</h2>
					<div class="soc-ico">
		';

		$html .= implode( '', $share_buttons );
...
if ( ! function_exists( 'presscore_display_share_buttons_for_image' ) ) :
	...
	function presscore_display_share_buttons_for_image( $place = '', $options = array() ) {
		...
		$options['class'][] = 'album-share-overlay';
		...
		if ( ! empty( $share_buttons ) ) {
			...
			$html .= '<div class="' . esc_attr( implode( ' ', $options['class'] ) ) . '">';
				$html .= '<a  class="share-button entry-share dt-btn-m' . $link_class . '" href="#">' . $button_title . '</a>';
				$html .= '<div class="soc-ico">';
					$html .= implode( '', $share_buttons );
				$html .= '</div>';
			$html .= '</div>';
...
if ( ! function_exists( 'dt_get_next_page_button' ) ) :
	...
	function dt_get_next_page_button( $max, $class = '' ) {
		...
		if ( $next_posts_link ) {
			...
			$icon = '<span class="stick"></span><span class="stick"></span><span class="stick"></span><span class="stick"></span><span class="stick"></span><span class="stick"></span>';
